...
debug_dump() {
  ...
  "$DC" -f "$APP_DIR/docker-compose.yml" ps || true
  "$DC" -f "$APP_DIR/docker-compose.yml" logs --tail=200 web db || true
...
ExecStart=/usr/bin/env bash -lc '$DC -f $APP_DIR/docker-compose.yml up -d'
ExecStop=/usr/bin/env bash -lc '$DC -f $APP_DIR/docker-compose.yml down'
ExecReload=/usr/bin/env bash -lc '$DC -f $APP_DIR/docker-compose.yml up -d --build'
TimeoutStartSec=0
